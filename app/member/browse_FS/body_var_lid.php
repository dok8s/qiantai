	...
	<div class="bet_select_content" id="bet_select">
		...
		<form name='lid_form'>
			<table class="bet_select_table" cellpadding="0" cellspacing="0" border="0">
				...
				$mDate=date('m-d');
				$fromName = 'foot_match';
				$where = " WHERE M_Start > now() AND m_Date ='$mDate' and r_show=1";
				switch($gtype){
					case 'BK':
						$fromName = 'bask_match';
						$where = " WHERE `M_Start` > now( ) AND `m_Date` ='$mDate'";
						break;
					case 'BS':
						$fromName = 'baseball';
						$where = " WHERE `M_Start` > now( ) AND `m_Date` ='$mDate' and r_show=3";
						break;
					case 'OP':
						$fromName = 'other_play';
						$where = " WHERE `M_Start` > now( ) AND `m_Date` ='$mDate' and r_show=3";
				...
				$mysql = "select distinct  M_League FROM $fromName $where";
			...
			<div allowtransparency="true" class="bet_select_center"><span class="bet_select_submit" onclick="chk_league();">送出</span></div>
		</form>
	</div>
</div>
