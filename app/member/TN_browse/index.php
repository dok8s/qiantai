...
$mysql = "select datasite,uid_tw,uid_en,runball,re_runball from web_system";
...
//alert(flash_ior_set);
		...
		function VideoFun(eventid, hot, play, gtype) {
			...
			if (play == "Y") {
				...
				tmpStr= "<span ><div style=\"cursor:hand\" class=\"bet_TV_btn_on\" onClick=\"parent.OpenLive('"+eventid+"','"+gtype+"')\"></div></span>";
			} else {
				//tmpStr+= "<img lowsrc=\"/images/member/video_2.gif\">";
				tmpStr= "<span ><div  class=\"bet_TV_btn_out\"></div></span>";
			}
			return tmpStr;
		}
		...
		function chkRepeat(gid,getDateTime){
			...
			for (var i=0 ; i < top.ShowLoveIarray[getGtype].length ; i++){
				if(top.ShowLoveIarray[getGtype][i][0]==gid && top.ShowLoveIarray[getGtype][i][1] == getDateTime)sw =true;break;
			}
			return sw;
		}
		...
		<script>
			...
			function getLayer(onelayer,gamerec,odd_f_type){
				...
				if (NoshowLeg[ObjDataFT[gamerec].league]==-1){
					...
					onelayer=onelayer.replace("*LegMark*","<span id='LegClose'></span>"); //聯说小D
				}else{
					onelayer=onelayer.replace(/\*CLASS\*/g,"style='display: ;'");
					onelayer=onelayer.replace("*LegMark*","<span id='LegOpen'></span>");  //聯说小D
				}
				//---------------------------------------------------------------------
				//Pr start
				var R_ior =Array();
				...
				onelayer=onelayer.replace("*TEAM_H*",ObjDataFT[gamerec].team_h.replace("[Mid]","<font color=\"#005aff\">[N]</font>").replace("[]","<font color=\"#005aff\">[]</font>"));
				...
				if (ObjDataFT[gamerec].strong=="H"){
					onelayer=onelayer.replace("*CON_RH*",ObjDataFT[gamerec].ratio);	/*讓^*/
				...
				onelayer=onelayer.replace("*RATIO_RH*",parseUrl(uid,odd_f_type,"H",ObjDataFT[gamerec],gamerec,"R"));/*讓r*/
				onelayer=onelayer.replace("*RATIO_RC*",parseUrl(uid,odd_f_type,"C",ObjDataFT[gamerec],gamerec,"R"));
				//小
				if (top.langx=="en-us"){
					onelayer=onelayer.replace("*CON_OUH*",ObjDataFT[gamerec].ratio_o.replace("O","<b>"+"o"+"</b>"));	/*小^*/
					onelayer=onelayer.replace("*CON_OUC*",ObjDataFT[gamerec].ratio_u.replace("U","<b>"+"u"+"</b>"));
				}else{
					onelayer=onelayer.replace("*CON_OUH*",ObjDataFT[gamerec].ratio_o.replace("O",top.strOver));	/*小^*/
					onelayer=onelayer.replace("*CON_OUC*",ObjDataFT[gamerec].ratio_u.replace("U",top.strUnder));
				}
				onelayer=onelayer.replace("*RATIO_OUH*",parseUrl(uid,odd_f_type,"C",ObjDataFT[gamerec],gamerec,"OU"));/*小r*/
				...
				if (ObjDataFT[gamerec].eventid != "" && ObjDataFT[gamerec].eventid != "null" && ObjDataFT[gamerec].eventid != undefined) {	//袛欠D
			...
			function parseUrl(uid,odd_f_type,betTeam,GameData,gamerec,wtype){
				...
				var ret="<a href='javascript://' onclick=\"parent.parent.mem_order.betOrder('TN','"+wtype+"','"+param+"');\" title='"+team+"'><font "+check_ioratio(gamerec,tmp_rtype,GameData)+" class='bet_bg_color'>"+ioratio+"</font></a>";//wtype
	...
	if($rtype=="re"){
		...
		<script>
			...
			function getLayer(onelayer,gamerec,odd_f_type){
				...
				var team_h = new Array();
				var team_hstr = team_h = ObjDataFT[gamerec].team_h.split("<font color=gray>");
				var team_cstr = ObjDataFT[gamerec].team_c.split("<font color=gray>");
				if(team_h[1]){
					team_h = team_h[1].split("(");
					team_h = team_h[1].split(")");
					team_h = '<span class="bet_round_bg"><tt class="bet_round_game"><font color=gray>'+team_h[0]+'</font></tt></span>';
				...
				if (ObjDataFT[gamerec].isMaster=="Y"){
					...
					if(ObjDataFT[gamerec].nowServer==0){
						onelayer=onelayer.replace("*NOWC*","class=\'bet_line_on\'");
					}
					if(ObjDataFT[gamerec].nowServer==1){
						onelayer=onelayer.replace("*NOWH*","class=\'bet_line_on\'");
					}
					onelayer=onelayer.replace("*NOWH*","class=\'bet_line_out\'");
					onelayer=onelayer.replace("*NOWC*","class=\'bet_line_out\'");
					...
					onelayer=onelayer.replace("*COUNT*",team_h);
					onelayer=onelayer.replace("*NODATA*","&nbsp;");
					//20150529 bill 補上天氣延賽
					onelayer=onelayer.replace("*SHOWDELAY*","style='display:none;'");
				}
				...
				onelayer=onelayer.replace("*TEAM_H*",team_hstr[0].replace("[Mid]","<font color=\"#005aff\">[N]</font>").replace("[]","<font color=\"#005aff\">[]</font>"));
				onelayer=onelayer.replace("*TEAM_C*",team_cstr[0]);
				onelayer=onelayer.replace("*SE*",top.str_RB);
				//全
				//讓
				if (ObjDataFT[gamerec].strong=="H"){
			...
			function parseUrl(uid,odd_f_type,betTeam,GameData,gamerec,wtype){
				...
				var ioratio=eval(ioratio_str);
				var ret="<a href='javascript://' onclick=\"parent.parent.mem_order.betOrder('TN','"+rewtype[wtype]+"','"+param+"');\" title='"+team+"'><font "+check_ioratio(gamerec,tmp_rtype,GameData)+" class='bet_bg_color'>"+ioratio+"</font></a>";//
	...
	if($rtype=="pd"){
		...
		<script>
			...
			function parseUrl(uid,odd_f_type,rtype,GameData,gamerec,wtype){
				...
				//alert(parent.name)
				var ret="<a href='javascript://' onclick=\"parent.parent.mem_order.betOrder('TN','"+wtype+"','"+param+"');\"  title='"+team+"'><font "+check_ioratio(gamerec,tmp_rtype,GameData)+" class='bet_bg_color'>"+ioratio+"</font></a>";
...
//	conscroll.style.display="none";
			...
			function parseUrl(uid,odd_f_type,betTeam,GameData,gamerec,wtype){
				...
				if (urlArray[wtype]!=null){
					...
					team = team.replace("<font color=gray>","").replace("</font>","");
...
//							body_browse.document.getElementById("euro_up").style.display="none";
	...
	<SCRIPT LANGUAGE="JAVASCRIPT">
		<!--
		//if(self == top) location='http://ra8888.com/app/member/';
...
<frameset rows="0,*" frameborder="NO" border="0" framespacing="0">
	<frame name="body_var" scrolling="NO" noresize src="body_var.php?uid=<?=$uid?>&rtype=<?php echo $rtype?>&langx=<?=$langx?>&mtype=<?=$mtype?>&delay=<?=$delay?>">
	<frame name="body_browse" src="body_browse.php?uid=<?=$uid?>&rtype=<?php echo $rtype?>&langx=<?=$langx?>&mtype=<?=$mtype?>&delay=<?=$delay?>">
</frameset>
