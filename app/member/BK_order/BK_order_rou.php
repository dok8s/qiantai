...
$filename="".$site."/app/member/BK_order/BK_order_rou.php?gid=$gid&uid=$suid&type=$gtype&gnum=$gnum&odd_f_type=H&langx=$langx";
...
$mysql = "select mb_team,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,concat('$body_mb_dimes',m_dime) as MB_Dime,concat('$body_tg_dimes',m_dime) as TG_Dime,MB_Dime_Rate,TG_Dime_Rate from bask_match where `MID`=$gid and cancel<>1 and fopen=1";// and now()>date_add(m_start,interval 40 second)
...
//userlog($memname);
	...
	if(!$ptype){
		...
		$sql="select sum(if((odd_type='I' and m_rate<0),abs(m_rate)*betscore,betscore)) as gold from web_db_io where m_date='$gdate' and m_name='$memname' order by id";/*姹傚嚭鏈棩绱*/
...
//	if ($M_Rate==0 or $M_Place==''or $M_Rate > 2){
//		wager_order($uid,$langx);
//	}
	/*if(number_format($M_Rate,2)!=number_format(change_rate($open,$pl),2)){
		if($pl<>''){
			switch ($gtype){
				case "H":
					$spl="MB_Dime_Rate='".$pl."'";
			...
			$up_sql="update bask_match set ".$spl." where `MID`='$gid'";
			mysql_db_query($dbname, $up_sql);
		}
		wager_order($uid,$langx);
		echo "<SCRIPT language='javascript'>parent.location='".BROWSER_IP."/app/member/select.php?uid=$uid&langx=$langx';</script>";
		exit;
...
?>
	...
	<form name="LAYOUTFORM" action="/app/member/BK_order/BK_order_rou_finish.php" method="post" onSubmit="return false">
		<div class="ord_main">
			<div class="ord_DIV">
				...
				<input type=hidden value="<?=$credit?>" name=restcredit><input type="hidden" name="odd_f_type" value="<?=$odd_type?>">
			</div>
	</form>
