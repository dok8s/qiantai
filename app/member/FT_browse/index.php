...
//	obj_msg.innerHTML = '<marquee scrolldelay=\"300\">'+msg+'</marquee>';
			...
			loadingOK();
			if(gameCount)showHOT(gameCount);
			try{
...
//20121023 max新增 輸水盤 負值顯示藍色
		...
		function parseAllBets(GameData,g_more){
			...
			if(g_more=='0'||GameData.more=='0'){
				...
				ret="<A href=javascript: onClick=parent.show_allbets('"+GameData.gid+"',event);><font class='bet_more_btn'>"+str_more+" ("+GameData.more+")</font></A>";
			}
			return ret;
		}
...
//	onelayer=onelayer.replace(/\*LegID\*/g,"LEG_"+legnum);
		...
		function parseUrl(uid,odd_f_type,betTeam,GameData,gamerec,wtype){
			...
			//var ret="<a href='"+order+"?"+param+"' target='mem_order' title='"+team+"'><font "+check_ioratio(gamerec,tmp_rtype,GameData)+">"+ioratio+"</font></a>";
			//alert(parent.name)
			var ret="<a href='javascript://' onclick=\"parent.parent.mem_order.betOrder('FT','"+rewtype[wtype]+"','"+param+"');\" title='"+team+"'><font "+check_ioratio(gamerec,tmp_rtype,GameData)+" class='bet_bg_color'>"+ioratio+"</font></a>";
		...
		function parseAllBets(GameData,g_more){
			...
			if(g_more=='0'||GameData.more=='0'){
				...
				ret="<A href=javascript: onClick=parent.show_allbets('"+GameData.gid+"',event,'"+datetime[0]+"');><font class='bet_more_btn'>"+str_more+" ("+GameData.more+")</font></A>";
			}
			return ret;
		}
...
?>
	...
	<SCRIPT LANGUAGE="JAVASCRIPT">
		...
		var str_even = 'Và';
		var str_renew = 'Tự động cập nhật sau vài giây';
		var str_submit = 'Xác nhận';
	...
	<frameset rows="0,*" frameborder="NO" border="0" framespacing="0">
		<frame name="body_var" scrolling="NO" noresize src="./body_var.php?uid=<?=$uid?>&rtype=<?=$rtype?>&langx=<?=$langx?>&mtype=<?=$mtype?>&delay=<?=$delay?>&league_id=<?=$league_id?>&hot_game=<?=$hot_game?>&page_no=<?=$page_no?>">
		<frame name="body_browse" src="./body_browse.php?uid=<?=$uid?>&rtype=<?=$rtype?>&langx=zh-vn&mtype=<?=$mtype?>&delay=<?=$delay?>&showtype=<?=$showtype?>">
	</frameset>
