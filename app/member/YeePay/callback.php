...
if ($bRet){
	...
	if ($r8_MP==""){
		...
		$sql="select * from web_member where memname = '".$r8_MP."'";
	...
	#	并且需要对返回的处理进行事务控制，进行记录的排它性处理，防止对同一条交易重复发货的情况发生.      	  	
		    ...
		    if ($r9_BType=="1"){
				$sql="select * from sys800 where Order_Code = '".$r6_Order."'";
				$result = mysql_db_query($dbname,$sql);
				$cou=mysql_num_rows($result);
				if ($cou==0){
				    $date=date("Y-m-d");
					$datetime=date("Y-m-d H:i:s");
				    $sql = "insert into sys800 set checked=1,payway='W',gold='$r3_Amt',AddDate='$date',type='S',Memname='$r8_MP',agents='$agents',world='$world',corprator='$corprator',super='$super',Admin='$admin',curtype='RMB',date='$datetime',name='$r8_MP',user='$r8_MP',waterno='".$_REQUEST['rb_BankId']."',Order_Code='$r6_Order'";
				    mysql_db_query($dbname,$sql);
				    $sql_amt = "update web_member set Credit=Credit+$r3_Amt,Money=Money+$r3_Amt  where Memname='$r8_MP'";
			...
			#如果需要应答机制则必须回写流,以success开头,大小写不敏感.
				...
				if ($cou==0){
				    ...
				    $sql = "insert into sys800 set checked=1,payway='W',gold='$r3_Amt',AddDate='$date',type='S',Memname='$r8_MP',agents='$agents',world='$world',corprator='$corprator',super='$super',Admin='$admin',curtype='RMB',date='$datetime',name='$r8_MP',user='$r8_MP',waterno='".$_REQUEST['rb_BankId']."',Order_Code='$r6_Order'";
				    mysql_db_query($dbname,$sql);
				    $sql_amt = "update web_member_data set Credit=Credit+$r3_Amt,Money=Money+$r3_Amt  where Memname='$r8_MP'";
				    mysql_db_query($dbname,$sql_amt);		
			        echo "success";
			        echo "<Script language=javascript>alert('交易成功,请回首页重新登入.');</script>";//window.open('http://ww060.com','_top')
				}
