...
$mysql = "select maxgold,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team, m_re_letb as M_LetB_HR_en,$m_league as M_League,ShowType,mb_re_letb_rate as MB_LetB_Rate_HR,tg_re_letb_rate as TG_LetB_Rate_HR,mb_ball as MB_Win,tg_ball as TG_Win from other_play where `MID`=$gid and cancel<>1 and fopen=1";// and now()>date_add(m_start,interval 40 second)
...
if($cou==0){
	...
	$ptype=$mrow['pay_type'];
	//userlog($memname);
	$GMAX_SINGLE=$mrow['OP_RE_Scene'];
	$GSINGLE_CREDIT=$mrow['OP_RE_Bet'];
	$open=$mrow['OpenType'];
