...
<HEAD><TITLE>123</TITLE>
	...
	<SCRIPT language=JavaScript>
		...
		<?php
            ...
            $base_url = "".$site."/app/member/TN_future/index.php?rtype=$rtype&uid=$suid&langx=$langx&mtype=$mtype";
            $filename="".$site."/app/member/TN_future/body_var.php?rtype=$rtype&uid=$suid&langx=$langx&mtype=$mtype&league_id=$league_id&page_no=".$page_no;
        ...
        case "r":
            /*
            $mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,M_LetB,$m_league as M_League,MB_Win,TG_Win,M_Flat,MB_Dime_Rate,TG_Dime_Rate,$mb_dime as MB_Dime,$tg_dime as TG_Dime,MB_LetB_Rate,TG_LetB_Rate,MB_MID,TG_MID,ShowType,M_Type,s_single,s_double from tennis where`m_start` > now( ) AND `m_Date` ='$mDate'".$league." and R_Show=1 and $mb_team<>'' order by m_start,mid";
            $result = mysql_db_query($dbname, $mysql);
        ...
        //echo $mysql;
            ...
            while ($row=mysql_fetch_array($result)){
                ...
                if ($row['M_Type']==1){
                    echo "parent.GameFT[$K]= Array('$row[MID]','$row[M_Date]<br>$row[M_Time]<br><font style=background-color=red>$run</font>','$row[M_League]','$row[MB_MID]','$row[TG_MID]','$row[MB_Team]','$row[TG_Team]','$row[ShowType]','$row[M_LetB]','$MB_LetB_Rate','$TG_LetB_Rate','$row[MB_Dime]','$row[TG_Dime]','$TG_Dime_Rate','$MB_Dime_Rate','$row[MB_Win]','$row[TG_Win]','$row[M_Flat]','$Odd','$Even','$s_single','$s_double','0','0');\n";
                }else{
                    echo "parent.GameFT[$K]= Array('$row[MID]','$row[M_Date]<br>$row[M_Time]','$row[M_League]','$row[MB_MID]','$row[TG_MID]','$row[MB_Team]','$row[TG_Team]','$row[ShowType]','$row[M_LetB]','$MB_LetB_Rate','$TG_LetB_Rate','$row[MB_Dime]','$row[TG_Dime]','$TG_Dime_Rate','$MB_Dime_Rate','$row[MB_Win]','$row[TG_Win]','$row[M_Flat]','$Odd','$Even','$s_single','$s_double','0','0');\n";
                }
            $K=$K+1;
            }
            */
        ...
        /*
            ...
            */
            ...
            for($i=0;$i<$cou;$i++){
                ...
                $sql = "update tennis set MB1TG0='$datainfo[8]',MB2TG0='$datainfo[9]',MB2TG1='$datainfo[10]',MB3TG0='$datainfo[11]',MB3TG1='$datainfo[12]',MB3TG2='$datainfo[13]',MB4TG0='$datainfo[14]',MB4TG1='$datainfo[15]',MB4TG2='$datainfo[16]',MB4TG3='$datainfo[17]',PD_Show=1 where MID=$datainfo[0]";
        ...
        */
        ...
        case "pr":
            ...
            while ($row=mysql_fetch_array($result)){
                ...
                echo "parent.GameFT[$K]= Array('$row[MID]','$pdate','$row[M_Sleague]','$row[MB_MID]','$row[TG_MID]','$mb_team','$row[TG_Team]','$row[ShowType]','$row[M_PR_LetB]','$row[MB_PR_LetB]','$row[TG_PR_LetB]','$row[MB_Dime]','$row[TG_Dime]','$row[MB_Dime_Rate]','$row[TG_Dime_Rate]');\n";
                $K=$K+1;
            }
