...
<head>
	<title>下注分割畫面</title>
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8"><script>var show_ior = '100';</script>
	<script>
...
//	obj_msg.innerHTML = '<marquee scrolldelay=\"300\">'+msg+'</marquee>';
			...
			function parseUrl(uid,odd_f_type,betTeam,GameData,gamerec,wtype){
				...
				//alert(parent.name)
				var ret="<a href='javascript://' onclick=\"parent.parent.mem_order.betOrder('OP','"+wtype+"','"+param+"');\" title='"+team+"'><font "+check_ioratio(gamerec,tmp_rtype,GameData)+" class='bet_bg_color'>"+ioratio+"</font></a>";
...
//	onelayer=onelayer.replace(/\*LegID\*/g,"LEG_"+legnum);
			...
			function parseUrl(uid,odd_f_type,betTeam,GameData,gamerec,wtype){
				...
				//var ret="<a href='"+order+"?"+param+"' target='mem_order' title='"+team+"'><font "+check_ioratio(gamerec,tmp_rtype,GameData)+">"+ioratio+"</font></a>";
				//alert(parent.name)
				var ret="<a href='javascript://' onclick=\"parent.parent.mem_order.betOrder('OP','"+rewtype[wtype]+"','"+param+"');\" title='"+team+"'><font "+check_ioratio(gamerec,tmp_rtype,GameData)+" class='bet_bg_color'>"+ioratio+"</font></a>";
...
//	onelayer=onelayer.replace(/\*LegID\*/g,"LEG_"+legnum);
				...
				//onelayer=onelayer.replace("*TD_RH_CLASS*",check_ioratio(gamerec,"ior_RH",ObjDataFT[gamerec]));/*?sytle*/
				//onelayer=onelayer.replace("*TD_RH_CLASS*","class='b_rig'");/*?sytle*/
				...
				//?
				if (ObjDataFT[gamerec].hstrong=="H"){
					onelayer=onelayer.replace("*CON_HRH*",ObjDataFT[gamerec].hratio);	/*?螹*/
					onelayer=onelayer.replace("*CON_HRC*","");
				}else{
					onelayer=onelayer.replace("*CON_HRH*","");
					onelayer=onelayer.replace("*CON_HRC*",ObjDataFT[gamerec].hratio);
				}
				onelayer=onelayer.replace("*RATIO_HRH*",parseUrl(uid,odd_f_type,"H",ObjDataFT[gamerec],gamerec,"HPR"));/*?攲薹*/
				onelayer=onelayer.replace("*RATIO_HRC*",parseUrl(uid,odd_f_type,"C",ObjDataFT[gamerec],gamerec,"HPR"));
				//湮苤
				onelayer=onelayer.replace("*CON_HOUC*",ObjDataFT[gamerec].hratio_o.replace("O",top.strOver));	/*湮苤螹*/
				onelayer=onelayer.replace("*CON_HOUH*",ObjDataFT[gamerec].hratio_u.replace("U",top.strUnder));
				onelayer=onelayer.replace("*RATIO_HOUC*",parseUrl(uid,odd_f_type,"C",ObjDataFT[gamerec],gamerec,"HPOU"));/*湮苤攲薹*/
				 ...
				 */
				if (ObjDataFT[gamerec].eventid != "" && ObjDataFT[gamerec].eventid != "null" && ObjDataFT[gamerec].eventid != undefined) {	//瓚岆瘁衄滖畦
					tmpStr= VideoFun(ObjDataFT[gamerec].eventid, ObjDataFT[gamerec].hot, ObjDataFT[gamerec].play, "FT");
					//alert(tmpStr);
					onelayer=onelayer.replace("*TV*",tmpStr);
				}
...
//							body_browse.document.getElementById("euro_up").style.display="none";
	...
	<frameset rows="0,*" frameborder="NO" border="0" framespacing="0">
		<frame name="body_var" scrolling="NO" noresize src="body_var.php?uid=<?=$uid?>&rtype=<?=$rtype?>&langx=<?=$langx?>&mtype=<?=$mtype?>&league_id=<?=$league_id?>">
		<frame name="body_browse" src="body_browse.php?uid=<?=$uid?>&rtype=<?=$rtype?>&langx=<?=$langx?>&mtype=<?=$mtype?>&league_id=<?=$league_id?>">
	</frameset>
