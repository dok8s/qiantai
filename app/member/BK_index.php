...
if ($showtype=="future"){
	$header="future";
	$body=BROWSER_IP."/app/member/BK_future/index.php?uid=$uid&langx=$langx&mtype=$mtype";
}else if($showtype=="fs"){
	$header="fs";
	$body=BROWSER_IP."/app/member/FS_browse/index.php?uid=$uid&langx=$langx&mtype=$mtype";
}else if($showtype=="nfs"){
	$header="nfs";
	$body=BROWSER_IP."/app/member/browse_FS/loadgame_R.php?uid=$uid&langx=$langx&mtype=$mtype";
}else{
	$header="";
	$body=BROWSER_IP."/app/member/BK_browse/index.php?uid=$uid&langx=$langx&mtype=$mtype";
}
?>
<html>
<head>
...
<frameset rows="118,*" cols="*" frameborder="NO" border="0" framespacing="0"> 
  <frame name="header" scrolling="NO" noresize src="<?=BROWSER_IP?>/app/member/BK_header.php?uid=<?=$uid?>&showtype=<?=$header?>&langx=<?=$langx?>&mtype=<?=$mtype?>" >
