...
$sql = "select Memname,VB_R_Scene,VB_R_Bet,OpenType,language,pay_type,Money from web_member where Oid='$uid' and oid<>'' and Status<>0";
...
if($cou==0){
	...
	$mysql = "select maxgold,mb_team as team,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowType,MB_LetB_Rate,TG_LetB_Rate,M_LetB from volleyball where `MID`=$gid and cancel<>1 and fopen=1";// `m_start`>now() and
...
if($cou==2){
	...
	?>
		...
		<form name="LAYOUTFORM" action="/app/member/VB_order/VB_order_r_finish.php" method="post" onSubmit="return false">
			<div class="ord_main">
				<div class="ord_DIV">
