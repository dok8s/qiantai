...
if(!in_array($rtype,array('hr','re','pd','hpd','t','f','p3'))){$rtype='r';}
...
//20121023 max新增 輸水盤 負值顯示藍色
			if (odd_f_type=="M" || odd_f_type=="I"){
				if (ioratio<0) ioratio="<font>"+ioratio+"</font>";
			...
			//alert(parent.name)
			ret = "<a href='javascript://' onclick=\"parent.parent.mem_order.betOrder('FT','"+wtype+"','"+param+"');\" title='"+team+"'><font "+check_ioratio(gamerec,tmp_rtype,GameData)+" class='bet_bg_color'>"+ioratio+"</font></a>";
		...
		function parseAllBets(GameData,g_more){
			...
			if(g_more=='0'||GameData.more=='0'){
				...
				ret="<A href=javascript: onClick=parent.show_allbets('"+GameData.gid+"',event); class='bet_more_btn'><font class='total_color'>"+top.str_all_bets+" ("+GameData.more+")</font></A>";
			}
			return ret;
		}
		...
		function show_allbets(gid,evt){
			...
			body_browse.document.getElementById('max_leg').style.display="none";
			body_browse.document.getElementById('more_window').style.display="";
...
?>
		...
		function parseUrl(uid,odd_f_type,rtype,GameData,gamerec,wtype){
			...
			//var ret="<a href='"+order+"?"+param+"' target='mem_order' title='"+team+"'><font "+check_ioratio(gamerec,tmp_rtype,GameData)+">"+ioratio+"</font></a>";
			//alert(parent.name)
			var ret="<a href='javascript://' onclick=\"parent.parent.mem_order.betOrder('FT','"+wtype+"','"+param+"');\" title='"+team+"'><font "+check_ioratio(gamerec,tmp_rtype,GameData)+" class='bet_bg_color'>"+ioratio+"</font></a>";
...
//	onelayer=onelayer.replace(/\*LegID\*/g,"LEG_"+legnum);
			...
			if (tmp_ior_po*1 >0 && tmp_ior_pe*1 > 0){
				if (top.langx=="en-us"){
					var rario_eoo=top.str_o+" "+parseUrl(uid,top.odd_f_type,"O",ObjDataFT[gamerec],gamerec,"P");
					var ratio_eoe=top.str_e+" "+parseUrl(uid,top.odd_f_type,"E",ObjDataFT[gamerec],gamerec,"P");
				}else{
...
?>
	...
	<frameset rows="0,*" frameborder="NO" border="0" framespacing="0">
		<frame name="body_var" scrolling="NO" noresize src="body_var.php?uid=<?=$uid?>&rtype=<?=$rtype?>&langx=<?=$langx?>&mtype=<?=$mtype?>&g_date=<?=$g_date?>&league_id=<?=$league_id?>&hot_game=<?=$hot_game?>">
		<frame name="body_browse" src="body_browse.php?uid=<?=$uid?>&rtype=<?=$rtype?>&langx=<?=$langx?>&mtype=<?=$mtype?>&showtype=<?=$showtype?>&g_date=<?=$g_date?>&hot_game=<?=$hot_game?>&league_id=<?=$league_id?>">
	</frameset>
