...
<head>
	...
	<script>
		...
		function liveTVClickEvent(){
			...
			if (eventid != "" && eventid != "null" && eventid != "undefined") {	//判斷<?=$shi?><?=$fou?>有轉播
...
//			cont_myMarket.innerHTML = fave_cont;
		...
		function fixMoreWindow(){
			...
			parent.document.getElementById('showdata').width = MFU.offsetWidth-5;
			parent.document.getElementById('showdata').height = MFU.offsetHeight-5;
...
//	var wtype_chk_ary = [];
		...
		function TV_title(){
			...
			//397.all bes-TV 幫隱藏 (CRM-191)
			if (TV_eventid != "" && TV_eventid != "null" && TV_eventid != undefined && top.showKR != 'Y') {	//判斷是否有轉播
				tv_bton.title = top.str_TV_FT;
				tv_bton.className = "bet_select_TV_close";
			}
			//170526 - 3079.新會員端-滾球/今日/早盤-內層盤面都多了tv圖示
			else {
				tv_bton.style.display="none";
				//tv_bton.title = top.str_TV_FT;
				//tv_bton.className = "bet_select_TV_close";
		...
		function mod_class_close(){
			for(k=1;k<mod_ary.length;k++){
				...
				//alert(open_it)
				if(open_it == false){
					document.getElementById(modName).className = "bet_all_special_none";
				}else if(mod == modName){
...
<iframe id="iorPHP" name="iorPHP1" src="aaa.php?uid=<?=$uid?>&langx=<?=$langx?>&date=<?=date('Y-m-d')?>& gtype=<?=$gtype?> & ltype=<?=$ltype?>&gid=<?=$gid?>" width="0" height="0"></iframe>
</body>
</html>
