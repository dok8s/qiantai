...
if ($act==1){
	...
	$SCRIPTARRAY='var scripts=new Array();var rtype="PR"';
	$CONTENT='';
	$TEAMCOUNT=0;
	for ($i=1;$i<$teamcount+1;$i++){
		...
		if (!empty($type)){
			...
			$sql="select mid,m_date,$gnum as gnum,$team as team,$teama,showtype,concat($m_league,'-','$HandicapParlay') as lea,$team_s as team_s,$ratio as ratio from tennis where mid=$gid";// and m_start>now()
			...
			$havesql="select sum(BetScore) as BetScore from web_db_io where m_name='$memname' and FIND_IN_SET($gid,MID)>0 and linetype=8 and active=4";
...
<body  id="OFTP3" class="bodyset" onLoad="onLoad();LoadSelect();" >
<form name="LAYOUTFORM" action="/app/member/TN_order/TN_order_pr_finish.php" method="post" onSubmit="return false">
	<div class="ord_main">
		<div class="ord_DIV">
			<h1><?=$zhgg?></h1>
			<div class="main">
