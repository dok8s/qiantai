...
$filename="".$site."/app/member/FT_order/FT_order_rouhc.php?gid=$gid&uid=$suid&rtype=$rtype&wtype=$wtype&type=$type&gnum=$gnum&odd_f_type=H&langx=$langx";
...
$mysql = "select maxgold,mb_ball,tg_ball,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,$ratio_ouho as ratio_ouho,$ratio_ouhu as ratio_ouhu,$ior_OUHO as ior_OUHO,$ior_OUHU as ior_OUHU from foot_match where `MID`=$gid and cancel<>1 and fopen=1";
...
if($cou==0){
	...
	$havesql="select sum(if((odd_type='I' and m_rate<0),abs(m_rate)*betscore,betscore)) as BetScore from web_db_io where M_Name='$memname' and MID='$gid' and (LineType=303 or LineType=313) and Mtype='$gtype'";
	...
	switch ($type){
	case "O"://大
		$M_Place=$body_mb_dimes.$row['ratio_ouho'];
		$M_Rate=$ior[0];
		break;
	case "U"://小
		$M_Place=$body_tg_dimes.$row['ratio_ouho'];
		$M_Rate=$ior[1];
		break;
	}
...
?>
	...
	<form name="LAYOUTFORM" action="/app/member/FT_order/FT_order_rouhc_finish.php" method="post" onSubmit="return false">
		<div class="ord_main">
			<div class="ord_DIV">
				...
				<input type="hidden" value="<?=$line_type?>" name=line_type>
