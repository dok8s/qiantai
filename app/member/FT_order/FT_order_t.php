...
$mysql = "select maxgold,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,S_Single,S_Double,S_0_1,S_2_3,S_4_6,S_7UP,ior_HEOE,ior_HEOO from foot_match where `MID`=$gid and cancel<>1 and fopen=1";// `m_start`>now() and
...
if($cou==0){
	...
	switch ($gtype){
		...
		case "HODD":
			...
			$M_Rate=change_rate($open,$row["ior_HEOO"]);
			$name1=$strOdd."/".$strEven;
			$GMAX_SINGLE=$GMAX_SINGLE1;
			$GSINGLE_CREDIT=$GSINGLE_CREDIT1;
		...
		case "HEVEN":
			...
			$name1=$strOdd."/".$strEven;
			$res_total="<font color='red'>".$body_sb."</font>";
			$M_Rate=change_rate($open,$row["ior_HEOE"]);
			$GMAX_SINGLE=$GMAX_SINGLE1;
			$GSINGLE_CREDIT=$GSINGLE_CREDIT1;
			//$caption=$DsOrder;
			$linetype=25;
...
//}
	...
	<form name="LAYOUTFORM" action="/app/member/FT_order/FT_order_t_finish.php" method="post" onSubmit="return false">
		<div class="ord_main">
			<div class="ord_DIV">
