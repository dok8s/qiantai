...
switch($wtype){
		case 'AM':
			$M_LetB='ior_AMN';
			$MB_LetB_Rate='ior_AMH';
			$TG_LetB_Rate='ior_AMC';
			$r=$kaichang." - 14:59".$fenzhong;
			break;
		case 'BM':
			$M_LetB='ior_BMN';
			$MB_LetB_Rate='ior_BMH';
			$TG_LetB_Rate='ior_BMC';
			$r="15:00 - 29:59".$fenzhong;
			break;
		case 'CM':
			$M_LetB='ior_CMN';
			$MB_LetB_Rate='ior_CMH';
			$TG_LetB_Rate='ior_CMC';
			$r="30:00 - ".$banchang;
			break;
		case 'DM':
			$M_LetB='ior_DMN';
			$MB_LetB_Rate='ior_DMH';
			$TG_LetB_Rate='ior_DMC';
			$r=$xiabanchangkaishi." - 59:59".$fenzhong;
			break;
		case 'EM':
			$M_LetB='ior_EMN';
			$MB_LetB_Rate='ior_EMH';
			$TG_LetB_Rate='ior_EMC';
			$r="60:00 - 74:59".$fenzhong;
			break;
		case 'FM':
			$M_LetB='ior_FMN';
			$MB_LetB_Rate='ior_FMH';
			$TG_LetB_Rate='ior_FMC';
			$r="75:00 - ".$quanchang;
			break;
...
$mysql = "select maxgold,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,$MB_LetB_Rate as MB_Win,$TG_LetB_Rate as TG_Win,$M_LetB as M_Flat from foot_match where `m_start`>now() and `MID`=$gid and cancel<>1 and fopen=1 and mb_team<>'' and mb_team_tw<>'' and mb_team_en<>''";
...
//userlog($memname);
	...
	$havesql="select sum(BetScore) as BetScore from web_db_io where m_name='$memname' and MID='$gid' and linetype=201";
...
<form name="LAYOUTFORM" action="/app/member/FT_order/FT_order_m15_finish.php" method="post" onSubmit="return false">
<div class="ord">
...
<input type="hidden" value="<?=$langx?>" name=langx>
	...
	<input type="hidden" value="201" name=line_type>
