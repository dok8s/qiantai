...
$filename="".$site."/app/member/FT_order/FT_order_rt.php?gid=$gid&uid=$suid&rtype=$gtype&odd_f_type=H&langx=$langx";
...
$mysql = "select maxgold,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,S_Single,S_Double,S_0_1,S_2_3,S_4_6,S_7UP,str_odd,str_even,ior_RT01,ior_RT23,ior_RT46,ior_ROVER,ior_HRT0,ior_HRT1,ior_HRT2,ior_HRTOV,ior_HREOO,ior_HREOE from foot_match where `MID`=$gid and cancel<>1 and fopen=1";
...
if($cou==0){
	...
	case "RODD":
		...
		$M_Rate=change_rate($open,$row["str_odd"]);
		$name1=$run." - ".$strOdd.$strEven;
	...
	case "REVEN":
		...
		$name1=$run." - ".$strOdd.$strEven;
		$M_Rate=change_rate($open,$row["str_even"]);
		$GMAX_SINGLE=$GMAX_SINGLE1;
		$GSINGLE_CREDIT=$GSINGLE_CREDIT1;
		//$caption=$DsOrder;
		$linetype=15;
	...
	case "HRODD":
		...
		$M_Rate=change_rate($open,$row["ior_HREOO"]);
	...
	case "HREVEN":
		...
		$name1="<font color=\"#FF0000\">".$body_sb."</font> - ".$run." - ".$strOdd.$strEven;
		$M_Rate=change_rate($open,$row["ior_HREOE"]);
		$GMAX_SINGLE=$GMAX_SINGLE1;
		$GSINGLE_CREDIT=$GSINGLE_CREDIT1;
		//$caption=$DsOrder;
		$linetype=35;
	...
	$havesql="select sum(BetScore) as BetScore from web_db_io where m_name='$memname' and MID='$gid' and linetype='$linetype'";
...
?>
	...
	<form name="LAYOUTFORM" action="/app/member/FT_order/FT_order_rt_finish.php" method="post" onSubmit="return false">
		<div class="ord_main">
			<div class="ord_DIV">
