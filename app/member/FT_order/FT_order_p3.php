...
//userlog($memname);
$GMAX_SINGLE=$row['FT_PC_Scene'];
$GSINGLE_CREDIT=$row['FT_PC_Bet'];
$team=0;
$css='';
$G_ID=0;
$pl='';
...
if ($act==1){
	...
	for ($i=1;$i<$teamcount+1;$i++){
		...
		if (!empty($type)){
			...
			$sql="select mid,m_date,$gnum as gnum,$team as team,$teama,showtype,$leagues as lea,$team_s as team_s,$ratio as ratio from foot_match where mid=$gid";// and m_start>now()
...
<form name="LAYOUTFORM" action="/app/member/FT_order/FT_order_p3_finish.php" method="post" onSubmit="return false">
	<div class="ord_main">
		<div class="ord_DIV">
			<h1><?=$zhgg?></h1>
			<div class="main">
