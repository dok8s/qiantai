	...
	<div class="bet_select_content" id="bet_select">
		...
		<form name='lid_form'>
			<table class="bet_select_table" cellpadding="0" cellspacing="0" border="0">
				...
				$mDate=date('m-d');
				if ($rtype<>'re'){
					$mysql = "select distinct M_League FROM `volleyball` WHERE `M_Start` > now( ) AND `m_Date` >'$mDate' and r_show=1";
				}else{
					$mysql = "select distinct M_League FROM `volleyball` WHERE `m_Date` >'$mDate' and re_show=1";
				}
