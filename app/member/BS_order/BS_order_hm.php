...
$mysql = "select maxgold,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Win,TG_Win,M_Flat from foot_match where `MID`=$gid and cancel<>1 and fopen=1";// `m_start`>now() and
...
if($cou==0){
	...
	$havesql="select sum(BetScore) as BetScore from web_db_io where M_Name='$memname' and MID='$gid' and LineType=11";// and mtype='$gtype'";
	...
	if ($active==2){
	$caption=str_replace($Soccer,trim($EarlyMarket),$SbdyOrder);
}else{
	$caption=$SbdyOrder;
}
...
?>
	...
	<form name="LAYOUTFORM" action="/app/member/BS_order/BS_order_hm_finish.php" method="post" onSubmit="return false">
		<div class="ord_main">
			<div class="ord_DIV">
