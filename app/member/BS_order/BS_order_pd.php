...
$mysql = "select maxgold,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowType,MB1TG0,MB2TG0,MB2TG1,MB3TG0,MB3TG1,MB3TG2,MB4TG0,MB4TG1,MB4TG2,MB4TG3,MB0TG0,MB1TG1,MB2TG2,MB3TG3,MB4TG4,OVMB,MB0TG1,MB0TG2,MB1TG2,MB0TG3,MB1TG3,MB2TG3,MB0TG4,MB1TG4,MB2TG4,MB3TG4 from foot_match where `MID`=$gid and cancel<>1 and fopen=1";// `m_start`>now() and
...
if ($active==2){
	$caption=str_replace($Soccer,trim($EarlyMarket),$BdOrder);
}else{
	$caption=$BdOrder;
}
?>
...
<script language="JavaScript" src="/js/ft_pd_order<?=$body_js?>.js"></script>
<form name="LAYOUTFORM" action="/app/member/FT_order/FT_order_pd_finish.php" method="post" onSubmit="return false">
  <div class="ord">
    ...
    <div id="info">
      ...
      <p class="team"><?=$m_league?>&nbsp;&nbsp;<?=$row["M_Date"]?><br><?=$MB_Team?>  <font color=#CC0000><?=$M_Sign?> </font>  <?=$TG_Team?> <br><em><?=$M_Place?></em> @ <strong><?=$M_Rate?></strong></p>
	...
	<input type=hidden value="4" name=line_type>
