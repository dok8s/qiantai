...
$uid=$_REQUEST['uid'];
$sql = "select status,cancel,hidden,super,ID,Money,Memname,Agents,world,corprator,OpenType,language,pay_type,BS_M_Scene,BS_M_Bet from web_member where Oid='$uid' and oid<>'' and Status<>0 order by ID";
...
if($cou==0){
	...
	$GMAX_SINGLE=$memrow['BS_M_Scene'];
	$GSINGLE_CREDIT=$memrow['BS_M_Bet'];
//userlog($memname);
	...
	$bet_type1=$bangqiubc.$quanchang.$duying;
	$bet_type1_tw=$bangqiubc_tw.$quanchang_tw.$duying_tw;
	$bet_type1_en=$bangqiubc_en.$quanchang_en1.$duying_en;
	$bet_type=$bangqiubc."<br>".$quanchang."<br>".$duying;
	$bet_type_tw=$bangqiubc_tw."<br>".$quanchang_tw."<br>".$duying_tw;
	$bet_type_en=$bangqiubc_en."<br>".$quanchang_en1."<br>".$duying_en;
