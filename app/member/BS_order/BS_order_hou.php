...
$mysql = "select maxgold,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,M_Dime,MB_Dime_Rate,TG_Dime_Rate from foot_match where `MID`=$gid and cancel<>1 and fopen=1";//`m_start`>now() and
...
if($cou==0){
	...
	switch ($gtype){
	case "C":
		$M_Place=$body_mb_dimes.$row['M_Dime'];
		$M_Rate=$ior[0];
		break;
	case "H":
		$M_Place=$body_tg_dimes.$row["M_Dime"];
		$M_Rate=$ior[1];
		break;
	}

	if ($active==2){
	$caption=str_replace($Soccer,trim($EarlyMarket),$SbdxOrder);
}else{
	$caption=$SbdxOrder;
}
...
?>
	...
	<form name="LAYOUTFORM" action="/app/member/BS_order/BS_order_hou_finish.php" method="post" onSubmit="return false">
		<div class="ord_main">
			<div class="ord_DIV">
				...
				<INPUT type=hidden value=13 name=line_type>
