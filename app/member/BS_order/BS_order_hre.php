...
$mysql = "select maxgold,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team, M_re_LetB as M_LetB_en,$m_league as M_League,Showtype,MB_re_LetB_Rate as MB_LetB_Rate,TG_re_LetB_Rate as TG_LetB_Rate,mb_ball,tg_ball from foot_match where `MID`=$gid and cancel<>1 and fopen=1";
...
if($cou==0){
	...
	$ptype=$mrow['pay_type'];
	$GMAX_SINGLE=$mrow['FT_RE_Scene'];
	$GSINGLE_CREDIT=$mrow['FT_RE_Bet'];
	$open=$mrow['OpenType'];
	...
	if(!$ptype){
		...
		$sql="select sum(if((odd_type='I' and m_rate<0),abs(m_rate)*betscore,betscore)) as gold from web_db_io where m_date='$gdate' and m_name='$memname' order by id";/*��������ۼ�*/
...
//	if ($Sign==''){
//		echo "<SCRIPT language='javascript'>alert('$stopbet!');self.location='".BROWSER_IP."/app/member/select.php?uid=$uid';</script>";
//		exit();
//	}
//	if ($M_Rate<0){
//		echo "<SCRIPT language='javascript'>alert('$stopbet!!');self.location='".BROWSER_IP."/app/member/select.php?uid=$uid';</script>";
...
?>
	...
	<form name="LAYOUTFORM" action="/app/member/BS_order/BS_order_hre_finish.php" method="post" onSubmit="return false">
		<div class="ord_main">
			<div class="ord_DIV">
				...
				<input type=hidden value=19 name=line_type>
