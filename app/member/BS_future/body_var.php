...
<?php 
	...
	$base_url = "".$site."/app/member/BS_future/index.php?rtype=$rtype&uid=$suid&langx=$langx&mtype=$mtype";
	$thisHttp = new cHTTP();
	$thisHttp->setReferer($base_url);
	$filename="".$site."/app/member/BS_future/body_var.php?rtype=$rtype&uid=$suid&langx=$langx&mtype=$mtype&mtype=$mtype&g_date=$g_date&league_id=$league_id&page_no=".$page_no;
...
$chs = new Chinese("UTF8","BIG5", $meg);
	...
	for($i=0;$i<$cou;$i++){
		...
		$tg_dime=str_replace('U','С',$datainfo[12]);
	...
	mysql_db_query($dbname,$sql) or die ("ާ@!");
...
?>
 ...
 //  top.SI2_mem_index.mem_order.location = '<?=BROWSER_IP?>/app/member/select.php?uid=<?=$uid?>&langx=zh-tw';
  if(parent.retime > 0)
