...
function getAnnouCount(){
    $.post('/app/member/account/scroll_history.php',
        {
            'uid':top.uid,
            'langx':top.langx,
            't_important':1,
        ...
        function (data) {
            if(data.state == 1){
                if(data.info > 0 ){
                    ...
                    document.getElementById('count_ann').innerHTML=data.info;
...
/*�L����ʾ--��getrecRB.php�ĽY�����Mȥ,ȥ�Д��Ƿ�record_RB�Ƿ���0,����Е��@ʾ�L��Dʾ*/
...
function hrefs(){
    window.open("./getVworld.php?langx="+top.langx+"&uid="+top.uid,"Vworld","width=1100,height=700,top=0,left=0,status=no,toolbar=no,scrollbars=yes,resizable=no,personalbar=no");
}

//更新信用額度max
function reloadCrditFunction(){
    $.post('/app/member/reloadCredit.php',
        {
            'uid':top.uid,
            'langx':top.langx
        },
...
function showMyAccount(type){
    ...
    var par = "width=1100,height=700,status=no,location=no";
    window.open(_url, "account", par);
}
//live
function showLive(paramObj){
    ...
    var par = "";
    par+="uid="+(paramObj["uid"]||top.uid);
    par+="&langx="+(paramObj["langx"]||top.langx);
    par+="&liveid="+(paramObj["liveid"]||top.liveid);
    par+="&autoOddCheck="+(paramObj["autoOddCheck"]||top.autoOddCheck);
    par+="&eventid="+(paramObj["eventid"]||"");
    par+="&eventlive="+(paramObj["eventlive"]||"");
    par+="&gtype="+(paramObj["gtype"]||top.betGtype);
    par+="&openlive=Y";
    top.openlive = "Y";

    var _url = util.getNowDomain()+"/app/member/live/live.php?"+par;
    var param = "width=826,height=543,top=0,left=0,status=no,toolbar=no,scrollbars=no,resizable=no,personalbar=no";

    trace("eventlive="+paramObj["eventlive"]+",eventid="+paramObj["eventid"]);

    var isOpen = true;
    var tooSmall = (document.getElementById("top_tv").style.display=="none")?true:false;


    if(!tooSmall){
        show_tv.document.all.time_list.style.display = 'none';
        show_tv.document.all.div_title.style.display = 'none';
        show_tv.document.all.div_body.style.display = 'none';
        show_tv.document.all.main_head_bak.style.display = 'none';
        //show_tv.document.all.main_bet.style.display = 'none';
        //document.getElementById("show_tv").contentWindow.wagerLinkTV(paramObj);
    }
    top.newWinObj["Live"] = window.open(_url, "Live", param);
    top.newWinObj["Live"].focus();

    //启动timer，判断子窗体是否关闭
    newWinTimer = window.setInterval("IfWindowClosed()", 500);
}
...
function autoOpen(){
    top.openlive = "N";
    show_tv.document.all.time_list.style.display = '';
    show_tv.document.all.div_title.style.display = '';
    show_tv.document.all.div_body.style.display = '';
    show_tv.document.all.main_head_bak.style.display = '';
}

var newWinTimer;
function IfWindowClosed() {
    //判断子窗体是否关闭
    if (top.newWinObj["Live"].closed == true) {
        window.clearInterval(newWinTimer);
        autoOpen();
    }
}